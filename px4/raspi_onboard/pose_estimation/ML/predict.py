...
class PoseEstimatorNode(Node):
    def __init__(self):
        ...
        self.model = joblib.load('pose_estimation_model_odom_rpy.pkl')
        ...
        # Odometry publisher
        self.publisher = self.create_publisher(Odometry, '/estimated/odometry', 10)
    ...
    def odom_callback(self, odom_msg: Odometry):
        if self.imu_data is not None:
            ...
            orientation_q = odom_msg.orientation

            # Convert quaternion to yaw
            imu_roll, imu_pitch, imu_yaw = self.euler_from_quaternion(orientation_q)

            # Prepare input features for the model
            input_features = np.array([[imu_accel_x, imu_accel_y, imu_accel_z, imu_gyro_x, imu_gyro_y, imu_gyro_z, imu_roll, imu_pitch, imu_yaw]])
