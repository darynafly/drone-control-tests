...
class WaypointFollower(Node):
    def __init__(self):
        ...
        self.tolerance_2d = 0.7
        self.tolerance_z = 0.7

        self.desired_takeoff_time = 8.0 # for altitude mode

        self.takeoff_height = 3.0
        self.takeoff_vel = 0.8
        self.land_vel = 0.8
        ...
        if path_mode == 0: #point
            ...
            self.waypoints = [
                [0.0, 0.0, self.takeoff_height], 
                [square_side, 0.0 , self.takeoff_height], 
                [0.0, 0.0, self.takeoff_height],
            ]
            ...
            self.waypoints = [
                ...
                [0.0, 0.0, self.takeoff_height], 
                [3.0, 0.0, self.takeoff_height], 
                [3.0, 3.0, self.takeoff_height], 
                [0.0, 3.0, self.takeoff_height], 
                [0.0, 0.0, self.takeoff_height],     
    ...
    def altitude_takeoff(self):
        self.set_mode('STABILIZED')

        while not self.arm_drone(True):
        ...
        while time.time() - self.time_start < self.desired_takeoff_time:
            print("takeoff")
            msg = AttitudeTarget()
