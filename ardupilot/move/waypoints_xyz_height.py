...
class WaypointFollower(Node):
    def __init__(self):
        ...
        self.tolerance_2d = 4.0
        self.tolerance_z = 2.0
        ...
        self.publisher_path = self.create_publisher(Path, 'ideal_path_step', 10)
        self.publisher_path_all = self.create_publisher(Path, 'ideal_path_all', 10)
        self.publisher_path_curr = self.create_publisher(Path, 'real_path', 10)
        ...
        self.path_current.header.frame_id = "odom"
    ...
    def visualise_path_current(self, waypoint):
        # Add a new waypoint to the path
        pose = PoseStamped()
        pose.header = Header()
        pose.header.stamp = self.get_clock().now().to_msg()  # Timestamp
        ...
        self.path_current.poses.append(pose)  # Add the pose to the path
        self.path_current.header.stamp = self.get_clock().now().to_msg()  # Update the path timestamp
        self.publisher_path_curr.publish(self.path_current)  # Publish the path
    ...
    def pose_callback(self, msg):
        ...
        self.visualise_path_current(self.current_position)
