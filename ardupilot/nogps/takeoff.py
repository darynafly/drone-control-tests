    ...
    def timer_callback(self):
        if self.armed:
            if time.time() - self.time_start < self.desired_takeoff_time:
                msg = AttitudeTarget()
                msg.thrust = self.takeoff_thrust # Maintain a stable hover

                # Roll, pitch, and yaw
                roll = 0.07  # Positive value moves right; negative moves left
                pitch = 0.4  # Positive value moves forward; negative moves back
                yaw = self.yaw  # Maintain current yaw
